...
import { getLoginSalt, login } from '@/api/user'
import md5 from 'md5'
import qs from 'qs'
  ...
  methods: {
    ...
    handleLogin() {
      this.$refs.loginForm.validate(valid => {
        if (valid) {
          this.loading = true
          getLoginSalt(this.loginForm.username).then(res => {
            if (res.code === 200) {
              // pw: MD5(MD5(明文密码).toUpperCase() + salt).toUpperCase()
              this.loginForm.password = md5(md5(this.loginForm.password).toUpperCase() + res.data).toUpperCase()
              login(qs.stringify(this.loginForm)).then((res) => {
                if (res.code === 200) {
                  // 设置TOKEN
                  const { token, tokenName, roles, permissions } = res.data
                  this.$store.commit('user/SET_TOKEN_KEY', tokenName)
